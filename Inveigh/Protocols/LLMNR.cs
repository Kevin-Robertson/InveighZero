...
namespace Inveigh
{
    class LLMNR
    {
        ...
        public static byte[] GetLLMNRResponse(string method, string ipVersion, string requestType, IPAddress sourceIPAddress, IPAddress destinationIPAddress, byte[] sourcePortData, byte[] payload)
        {
            ...
            byte[] requestLength = new byte[1];      
            Buffer.BlockCopy(payload, 12, requestLength, 0, 1);
            byte[] request = new byte[(int)requestLength[0]];
            Buffer.BlockCopy(payload, 13, request, 0, request.Length);
            ...
            using (MemoryStream memoryStream = new MemoryStream())
            {
                ...
                if (String.Equals(requestType, "A"))
                {
