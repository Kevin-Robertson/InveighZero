...
namespace Inveigh
{
    class LLMNR
    {
        ...
        public static byte[] GetLLMNRResponse(string type, string ipVersion, string llmnrTTL, IPAddress sourceIPAddress, IPAddress destinationIPAddress, byte[] spooferIPData, byte[] spooferIPv6Data, byte[] udpSourcePort, byte[] udpPayload)
        {
            byte[] ttlLLMNR = BitConverter.GetBytes(Int32.Parse(llmnrTTL));
            Array.Reverse(ttlLLMNR);
            byte[] llmnrType = new byte[2];
            ...
            {
                llmnrMemoryStream.Write((new byte[2] { 0x14, 0xeb }), 0, 2);
                llmnrMemoryStream.Write(udpSourcePort, 0, 2);
                llmnrMemoryStream.Write((new byte[2] { 0x00, 0x00 }), 0, 2);
                llmnrMemoryStream.Write((new byte[2] { 0x00, 0x00 }), 0, 2);
            }
